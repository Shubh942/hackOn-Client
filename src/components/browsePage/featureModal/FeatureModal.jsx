...
import Personal_Recommendation from "./Personal_Recommendation";

export default function FeatureModal({ show, setShow, details, mediaType }) {
        ...
        .then(
          axios.spread((...res) => {
            ...
            let trailerData = mediaDetailRes?.videos?.results.find(
              (vid) =>
                vid.official === true &&
                vid.type.toLowerCase() === trailerString.toLowerCase()
            );

            setMediaTrailer(
              trailerData ? trailerData : mediaDetailRes?.videos?.results[0]
            );
          })
        )
        .catch((err) => {
  ...
  return (
    <>
      <Modal
        size="lg"
        show={show}
        onHide={handleModelCloseButton}
        className="m-0 modal-container"
      >
        <Modal.Body className="p-0" id="modal-content">
          ...
          <div className="content">
            ...
            <div className="crewInfo">
              ...
              {creators?.length >= 1 ? <Creators creators={creators} /> : null}
            </div>
          </div>
          {/* {console.log("ok")}
          {console.log(details)} */}
